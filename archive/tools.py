"""
A set of useful tools for handling the text segmentation tasks.
"""
import numpy as np
from itertools import chain, tee, zip_longest  # zip_longest in Python 3 for the izip behavior
from nltk.stem import PorterStemmer

stopword_set = set()

with open("/Users/ericdo/Documents/CDS/Paper Moderation/segmentation-master/data/STOPWORD.list") as f:
    for line in f:
...
def choi_loader(doc, tp, ref, word_cut=0, remove_stop=False, stem=False):
    ...
    doc = [[
        [x.lower() for x in sent.split() if is_valid(x)] for sent in doc
    ] for doc in parts]
...
ARX_TEMPLATE = "/Users/ericdo/Documents/CDS/Paper Moderation/segmentation-master/data/arxiv/{:03d}.ref"
def arx_loader(num):
    ...
    return [[
        [x] for x in x.split()
    ] for x in doc.split("BR")]
"""
def allchoi(set, *args, **kwargs):
    if set == "3-5":
        for a in [1, 2]:
            for i in range(50):  # Use range() instead of xrange() in Python 3
    ...
    if set == "3-11":
        ...
        for i in range(300):  # Use range() instead of xrange() in Python 3
...
testdoc = [
    ["this is the first sentence".split()] * 5, 
...
def pairwise(iterable):
    ...
    a, b = tee(iterable)
    next(b, None)
    return zip(a, b)  # Use zip() in Python 3 instead of izip()
...
def score_wd(hyp, ref, k=None):
    ...
    score = 0.0
    tot = 0.0
    for i in range(length - k):  # Use range() instead of xrange() in Python 3
