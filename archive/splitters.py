...
from functools import reduce  # for Python 3, `reduce` is in `functools`
import tools
...
def greedysplit(n, k, sigma):
    ...
    while k > 0:
        usedinds = set(splits)
        new = min(
            ((score(splits + [i], sigma), splits + [i]) for i in range(1, n) if i not in usedinds),
            key=lambda x: x[0]
        )
        splits = new[1]
        s = new[0]
...
def bestsplit_general(splits, pk, sigma, combine=lambda a, b: a + b, key=lambda a: a):
    ...
    best = min(((score(splits[:pk] + [j] + splits[pk + 1:], sigma), j)
                for j in range(left + 1, right)), key=lambda x: x[0])
    return best[1]
